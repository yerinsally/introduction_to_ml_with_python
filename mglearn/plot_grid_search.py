...
def plot_cross_val_selection():
    ...
    plt.legend([marker_cv, marker_mean, marker_best],
               ["크로스 밸리데이션 정확도", "평균 정확도", "최적 파라미터 세팅"], loc=(1.05, .4))
...
def plot_grid_search_overview():
    ...
    def draw(ax, text, start, target=None):
        ...
        annotation = ax.annotate(text, end, start, xycoords='axes pixels',
                                 textcoords='axes pixels', size=10,
    ...
    final_evaluation = draw(axes, "최종 평가", (5 * step, grr - 3 * step))
    retrained_model = draw(axes, "최종 모델 학습", (3 * step, grr - 3 * step), final_evaluation)
    best_parameters = draw(axes, "최적 파라미터", (.5 * step, grr - 3 * step), retrained_model)
    cross_validation = draw(axes, "크로스 밸리데이션", (.5 * step, grr - 2 * step), best_parameters)
    parameters = draw(axes, "파라미터 그리드", (0.0, grr - 0), cross_validation)
    training_data = draw(axes, "훈련 데이터", (2 * step, grr - step), cross_validation)
    draw(axes, "훈련 데이터", (2 * step, grr - step), retrained_model)
    test_data = draw(axes, "테스트 데이터", (5 * step, grr - step), final_evaluation)
    draw(axes, "데이터 세트", (3.5 * step, grr - 0.0), training_data)
    data_set = draw(axes, "데이터 세트", (3.5 * step, grr - 0.0), test_data)
    plt.ylim(0, 1)
    plt.xlim(0, 1.5)
